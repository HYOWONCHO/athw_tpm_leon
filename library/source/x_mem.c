...
//
static int x_memcmp_s(const void *dest, int dmax, 
                    const void *src, int smax, int *diff)
{
    const uint8_t *dp; const uint8_t *sp;

    dp = dest;
    sp = src;

    X_RET_VAL_IF_FAIL(diff != NULL, -ESNULLP);
    *diff = -1;

    X_RET_VAL_IF_FAIL(dp != NULL, -ESNULLP);
    X_RET_VAL_IF_FAIL(src != NULL, -ESNULLP);
    X_RET_VAL_IF_FAIL(((dmax > 0) && (smax > 0)), -ESZEROL);
    X_RET_VAL_IF_FAIL((smax < dmax), -ESLEMAX);
...
void *x_memcreate(size_t size)
{
    ...
    if(size >- sizeof(void *)) {
        retbuf - (void *)(((uintptr_t)ibuffer + sizeof(void *) - 1) & ~(sizeof(void *) -1));
...
/**
 * \fn int x_memcmp(const void *dest,  const void *src,  int n)
 *
 * \brief compare the byte string src against bystring dest 
 *
 * \param [in]dest  pointer to destnation buffer
 * \param [in]src   pointer to source buffer
 * \param [in]dest  pointer to destnation buffer
 *
 * \return on success, return 0, otherwise value is error 
 */
int x_memcmp(const void *dest,  const void *src,  int n)
{

    int diff = 0L;

    x_memcmp_s(dest, n, src, n, &diff) ;
